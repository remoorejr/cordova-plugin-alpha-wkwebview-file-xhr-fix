...
"use strict";
(function () {
	// Guard against double-installation.  This file is injected at document-start (as a native
	// WKUserScript, so the override is active before any app request) AND may also run again later as a
	// Cordova js-module; the guard ensures the XMLHttpRequest/fetch override is only wired up once per
	// document (re-wrapping would corrupt window._XMLHttpRequest).
	if (window.__alphaXhrInstalled) {
		return;
	}
	window.__alphaXhrInstalled = true;

	// Lazily resolve cordova/exec so this file can run at document-start, before cordova.js has loaded.
	// Only the file:// FileHandler and the getConfig fallback use it; the HTTP interception path uses
	// window.webkit.messageHandlers.nativeXHR and requires no Cordova.
	function exec() {
		var fn = null;
		if (typeof require === "function") {
			try {
				fn = require("cordova/exec");
			} catch (e) {
				fn = null;
			}
		}
		if (!fn && window.cordova && typeof window.cordova.require === "function") {
			try {
				fn = window.cordova.require("cordova/exec");
			} catch (e) {
				fn = null;
			}
		}
		...
		console.log("xhr-polyfill.js - cordova/exec unavailable; request could not be completed natively.");
	}
	...
	FileHandler._getMimeType = function (reqContext) {
		...
		else if (reqContext.responseHeaders["content-type"]) return reqContext.responseHeaders["content-type"];
	...
	FileHandler.getHandlerForResponseType = function (reqContext) {
		...
		if (FileHandler._RESPONSE_HANDLERS[responseType]) return FileHandler._RESPONSE_HANDLERS[responseType];
	...
	FileHandler.prototype.send = function () {
		...
		if (this._isTraceLoggingEnabled()) console.log("xhr-polyfill.js - native file XHR Request:\n%o", reqContext.url);

		FileHandler._presend(reqContext);

		exec(
			FileHandler._success.bind(this, reqContext, rspTypeHandler),
			FileHandler._error.bind(this, reqContext),
			"CDVWKWebViewFileXhr",
			rspTypeHandler.action,
			[reqContext.url],
		);
	};
	...
	HttpHandler._REQUEST_HANDLERS = {
		...
		ArrayBuffer: {
			convert: function (reqContext) {
				...
				if (!type) type = reqContext.requestHeaders["content-type"] = "application/octet-binary;";
			...
			convert: function (reqContext) {
				...
				var promise = new Promise(function (resolve, reject) {
					...
					body.__getRequestParts().then(function (parts) {
						if (!contentType) reqContext.requestHeaders["content-type"] = parts.contentType;
		...
		string: {
			convert: function (reqContext) {
				if (!reqContext.requestHeaders["content-type"]) reqContext.requestHeaders["content-type"] = "text/plain;";
		...
		Object: {
			convert: function (reqContext) {
				if (!reqContext.requestHeaders["content-type"]) reqContext.requestHeaders["content-type"] = "application/json;";
	...
	HttpHandler._convertEncode64StringToText = function (mimeType, encoding, r) {
		...
		for (var i = 0; i < binaryString.length; i++) byteNumbers[i] = binaryString.charCodeAt(i);
	...
	HttpHandler._getHandlerForRequestBodyType = function (reqContext) {
		var body = reqContext.requestData;
		if (!body) return HttpHandler._REQUEST_HANDLERS[""];
		else if (typeof body === "string") return HttpHandler._REQUEST_HANDLERS["string"];
		else if (Blob.prototype.isPrototypeOf(body)) return HttpHandler._REQUEST_HANDLERS["Blob"];
		else if (FormData.prototype.isPrototypeOf(body)) return HttpHandler._REQUEST_HANDLERS["FormData"];
		else if (ArrayBuffer.prototype.isPrototypeOf(body) || body.buffer)
			return HttpHandler._REQUEST_HANDLERS["ArrayBuffer"];
		else if (Document.prototype.isPrototypeOf(body)) return HttpHandler._REQUEST_HANDLERS["Document"];
		else return HttpHandler._REQUEST_HANDLERS["Object"];
	};

	HttpHandler._getHandlerForResponseType = function (reqContext) {
		var responseType = reqContext.responseType;
		if (HttpHandler._RESPONSE_HANDLERS[responseType]) return HttpHandler._RESPONSE_HANDLERS[responseType];
		else return HttpHandler._RESPONSE_HANDLERS["text"];
	};
		...
		handler
			...
			.then(
				function (bodyAsBase64String) {
					...
					var reqPayLoad = {
						id: id,
						callback: "nativeXHRResponse",
						url: HttpHandler._resolveUri(reqContext.url),
						method: reqContext.method,
						headers: reqContext.requestHeaders,
						body: bodyAsBase64String,
						timeout: timeoutInSecs,
					};

					if (this._isTraceLoggingEnabled()) console.log("xhr-polyfill.js - native XHR Request:\n %o", reqPayLoad);
			...
			.catch(HttpHandler._error.bind(this, reqContext));
	};
	...
	HttpHandler._error = function (reqContext, error, underlyingErrorCode) {
		...
		if (isTimeout) reqContext.dispatchProgressEvent("timeout");
		else reqContext.dispatchProgressEvent("error");
	...
	HttpHandler.prototype.load = function (payload) {
		...
		if (this._isTraceLoggingEnabled()) console.log("xhr-polyfill.js - native XHR Response:\n%o", payload);

		if (payload.error) HttpHandler._error(reqContext, payload.error, payload["underlyingErrorCode"]);
		else {
			...
			handler
				.convert(mimeType, encoding, payload.data)
				.then(HttpHandler._success.bind(this, reqContext, payload, mimeType));
		}
	};
	...
	DelegateHandler._FROM_PROPERTIES = ["response", "responseText", "responseXML", "responseURL", "status", "statusText"];
	...
	DelegateHandler.prototype.send = function () {
		...
		if (reqContext.overrideMimeType) delegate.overrideMimeType(reqContext.overrideMimeType);
		...
		for (var i = 0; i < keys.length; i++) delegate.setRequestHeader(keys[i], reqContext.requestHeaders[keys[i]]);

		var requestData = reqContext.requestData;
		reqContext.requestData = undefined;

		// returns a native FormData from the plugin's polyfill
		if (FormData.prototype.isPrototypeOf(requestData)) requestData = requestData.__getNative();
	...
	HandlerFactory._getConfig = function () {
		...
		if (HandlerFactory._config) {
		...
		} else if (window.__alphaXhrConfig) {
			// Config injected natively alongside the document-start user script; avoids needing cordova/exec
			// before cordova.js has loaded.
			HandlerFactory._config = window.__alphaXhrConfig;
			promise = Promise.resolve(HandlerFactory._config);
		} else {
			promise = new Promise(function (done) {
				...
				function error() {
					HandlerFactory._config = { InterceptRemoteRequests: "secureOnly", NativeXHRLogging: "none" };
					done(HandlerFactory._config);
				}
	...
	HandlerFactory.getHandler = function (context) {
		var promise = new Promise(function (resolve) {
			HandlerFactory._getConfig().then(function (config) {
				...
				if (noS3Intercepts && context.url.includes("s3.amazonaws.com") && context.url.includes("X-Amz-SignedHeaders")) {
					context.interceptRemoteRequests = "none";
				}

				if (context.interceptRemoteRequests)
					// backdoor to override per instance
				...
				if (
					...
					else resolve(new DelegateHandler(context, config));
				}
			});
		});
	...
	["readyState", "response", "responseText", "responseURL", "responseXML", "status", "statusText", "upload"].forEach(
		function (propName) {
			Object.defineProperty(window.XMLHttpRequest.prototype, propName, {
				get: function () {
	...
	["ontimeout", "onloadstart", "onprogress", "onabort", "onerror", "onload", "onloadend"].forEach(function (propName) {
		Object.defineProperty(window.XMLHttpRequest.prototype, propName, {
			get: function () {
				return this._context[propName];
			},
	...
	window.XMLHttpRequest.prototype.getAllResponseHeaders = function () {
		...
		var list = [];
		for (var i = 0; i < names.length; i++) list.push([names[i], responseHeaders[names[i]]].join(":"));
	...
	window.XMLHttpRequest.prototype.send = function (data) {
		if ("GET" !== this._context.method && "HEAD" !== this._context.method) this._context.requestData = data;
