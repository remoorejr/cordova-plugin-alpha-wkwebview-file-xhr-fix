...
"use strict";
(function () {
	// Guard against double-execution (document-start user script + Cordova js-module).
	if (window.__alphaFormDataInstalled) {
		return;
	}
	window.__alphaFormDataInstalled = true;
	function __FormData(formElement) {
		this._map = new Map();

		if (!HTMLFormElement.prototype.isPrototypeOf(formElement)) return;
		...
		for (var i = 0; i < elements.length; i++) {
			...
			if (element.type === "file") {
				for (var file of element.files) this.append(name, file);
			} else if (["select-multiple", "select-one"].indexOf(element.value) > -1) {
				for (var option of element.selectedOptions) this.append(name, option);
	...
	__FormData.prototype.values = function () {
		...
		while (entry && !entry.done) {
			...
			for (var i = 0; i < values.length; i++) allvalues.push(values[i]);
	...
	__FormData.prototype.__getNative = function () {
		...
		while (!entry.done) {
			...
			else fd.append(entry.value[0], entry.value[1]);
	...
	__FormData._getMultipartRequest = function (entries) {
		...
		for (var i = 0; i < entries.length; i++) {
			var entry = entries[i];
			for (var data of __FormData._generateMultipartFormData(entry, boundary)) bodyEntries.push(data);
		}
	...
	__FormData._makeIterator = function (array) {
		...
		return {
			next: function () {
				if (i < array.length) return { done: false, value: array[i++] };
	...
	__FormData._normalizeEntry = function (entry) {
		var promise = new Promise(function (resolve) {
			...
			if (value && (Blob.prototype.isPrototypeOf(value) || File.prototype.isPrototypeOf(value))) {
